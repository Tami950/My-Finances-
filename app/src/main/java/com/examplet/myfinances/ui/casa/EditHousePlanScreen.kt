...
fun EditHousePlanScreen(
    ...
    AppScreen {
        LazyColumn(contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            if (state.isLoading) {
                ...
                item(key = "month") { Text(monthLabel(state.month, state.year), style = MaterialTheme.typography.headlineSmall) }
                if (state.isClosed) item(key = "closed") {
                    Text(stringResource(R.string.house_month_closed_edit_blocked), color = MaterialTheme.colorScheme.error)
                }
                ...
                when (state.mode) {
                    HousePlanEditMode.PLAN -> {
                        ...
                        items(state.categories, key = { "edit-plan-category-${it.categoryId}" }) { row ->
                            AppContentCard(
                                modifier = Modifier.clickable(enabled = !state.isClosed) { viewModel.openCategory(row.categoryId) }
                            ) {
                                Row(modifier = Modifier.fillMaxWidth()) {
                                    Text(row.name, style = MaterialTheme.typography.titleMedium, modifier = Modifier.weight(1f))
                                    Text(
                                        formatHouseCents(
                                            if (row.categoryBehavior == HouseCategoryBehavior.FIXED_EXPENSE) row.plannedFixedCents
                                            else parseCentsOrZero(row.openingBalanceText) + parseCentsOrZero(row.allocatedText)
                                        )
                                    )
                                }
                                if (row.categoryBehavior == HouseCategoryBehavior.FIXED_EXPENSE) {
                                    ...
                                    Text(
                                        stringResource(
                                            ...
                                            formatHouseCents(row.fixedExpensePrefundedCents),
                                            formatHouseCents(row.effectiveAllocatedCents)
                                        ),
                        ...
                        item(key = "plan-summary") {
                            AppContentCard {
                                ...
                                if (state.allocationOverflowCents > 0) Text(
                                    stringResource(R.string.house_allocation_overflow_error, formatHouseCents(state.allocationOverflowCents)),
                                    color = MaterialTheme.colorScheme.error,
                                    style = MaterialTheme.typography.bodySmall
                                )
                                if (state.existingPositionOverflowCents > 0) Text(
                    ...
                    HousePlanEditMode.POSITIONS -> {
                        item(key = "position-help") {
                            AppContentCard {
                                Text(stringResource(R.string.house_positions_current_help), style = MaterialTheme.typography.bodyMedium)
                                SummaryRow(stringResource(R.string.house_summary_total_funds), state.totalHouseFundsCents)
                            }
                        }
                        items(state.positions, key = { "edit-position-${it.account.id}" }) { row ->
                            AppContentCard(
                                modifier = Modifier.clickable(enabled = !state.isClosed) { viewModel.openAccount(row.account.id) }
                            ) {
                                Row(modifier = Modifier.fillMaxWidth()) {
                        ...
                        item(key = "positions-summary") {
                            AppContentCard {
                                ...
                                if (state.positionOverflowCents > 0) Text(
                                    stringResource(R.string.house_position_overflow_error, formatHouseCents(state.positionOverflowCents)),
                                    color = MaterialTheme.colorScheme.error,
                                    style = MaterialTheme.typography.bodySmall
                                )
                            }
    ...
    if (state.mode == HousePlanEditMode.PLAN) {
        state.categories.firstOrNull { it.categoryId == state.selectedCategoryId }?.let { row ->
            AppModalBottomSheet(
                ...
                if (row.categoryBehavior == HouseCategoryBehavior.FIXED_EXPENSE) {
                    OutlinedTextField(
                        ...
                        onValueChange = { viewModel.updateFixedExpensePlanned(row.categoryId, it) },
                    ...
                    if (row.fixedExpensePrefundedCents > row.plannedFixedCents) Text(
                        stringResource(R.string.house_fixed_expense_prefunded_overflow),
                        color = MaterialTheme.colorScheme.error
                    )
                } else {
                    OutlinedTextField(
    ...
    if (state.mode == HousePlanEditMode.POSITIONS) {
        state.positions.firstOrNull { it.account.id == state.selectedAccountId }?.let { row ->
            AppModalBottomSheet(
                ...
                OutlinedTextField(
                    ...
                    trailingIcon = {
                        if (row.amountText.isNotEmpty()) {
                            IconButton(onClick = { viewModel.updateAccountAmount(row.account.id, "") }) {
                                Icon(Icons.Outlined.Close, contentDescription = stringResource(R.string.action_clear_amount))
                            }
                        }
                    },
                    singleLine = true,
